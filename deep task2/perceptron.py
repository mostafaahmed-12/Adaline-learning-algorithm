...
#mariam
def perceptron(x_training__, y_training__, weights__, learning_rate, epochs,mse_threshold):
    w = weights__
    x = x_training__
    d = y_training__
    sumoferor=0
    for j in range(0,epochs):
     for i in range(0, 60):
      e=np.array(x[i])
      net=np.dot(e,np.transpose(w))
      erorr=d[i]-net[0]
      w=w+learning_rate*erorr*e
     for a in range(0,60):
      o=np.array(x[a])
      net=np.dot(o,np.transpose(w))
      sumoferor+=(d[a]-net[0])*(d[a]-net[0])*0.5
     meansquare=(1/60)*sumoferor
     if meansquare<=mse_threshold:
         print(meansquare)
         break
     else:
        sumoferor=0
...
def tarin_and_test_slicing__for_gui(class_no1, class_no2, f1_name, f2_name, learning_rate, epochs, boolean_bias,mes):
    ...
    y_testing = y_test
    final_weights = perceptron(x_training, y_training, weights, learning_rate, epochs,mes)
